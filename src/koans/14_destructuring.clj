...
(meditations
  ...
  (= (str "First comes love, "
     ...
     ((fn [[a b c]] (str "First comes " a ", then comes " b ", then comes Clojure with the baby carriage"))
  ...
  (= {:original-parts ["Steven" "Hawking"] :named-parts {:first "Steven" :last "Hawking"}}
     (let [[first-name last-name :as full-name] ["Steven" "Hawking"]]
       {:original-parts [first-name last-name] :named-parts { :first (first full-name) :last (nth full-name 1) }}))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [[first-name last-name] {street-address :street-address city :city state :state}]
